...
import { update_testsuite, projects_names, modules_by_project_id, get_detail_testsuite } from '../../api/api';
...
export default {
    ...
    methods: {
        ...
        getInitModulesByProjectID(pro_id) {
            modules_by_project_id(pro_id)
                .then((response) => {
                    this.unselected = response.data;
                    if (this.selected.length != 0) {
        ...
        getTestSuiteDetail() {
            get_detail_testsuite(this.current_testsuite_id)
                .then((response) => {
                    ...
                    this.form.project = response.data.project_id;
                    this.before_project = this.form.project;
                    this.selected = response.data.include;
                    this.getInitModulesByProjectID(this.form.project);
                })
                .catch((error) => {
                    this.$message.error('服务器错误');
                });
